...
namespace rayCpp
{
    ...
    void DrawSprite(const Texture2D& s, const Vec2& pos, const RectF& srcRect, const float wDest, const float hDest, Color c, const float ro);
    ...
    void DrawSpriteInside(const Texture2D& s, const Vec2& pos, const RectF& srcRect, const RectF& dest, const RectF& boundary, Color color = WHITE);
    ...
    RectF GetScreenRect();
    Vec2 GetCenterScreen();
    Vec2 GetMousePos();
    Vec2 CreateVecByData( const std::string& data, const char delim );
    Vei2 CreateVeiByData( const std::string& data, const char delim );
