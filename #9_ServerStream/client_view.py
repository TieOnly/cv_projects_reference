...
class App():
	...
	def active_event(self):
		for event in pygame.event.get():
			...
			if event.type == KEYUP:
				...
				if event.key == K_r:
					self.io_put(tcp_pack(DT.STR, "task-view"))
				if event.key == K_t:
					self.io_put(tcp_pack(DT.SERIES, {'id':SERIES_ID.DATA, 'd':{1,2}}))
	...
	def process_data_series(self, data):
		...
		if id == SERIES_ID.CAM:
			try:
				sock_id, frame = d['sk'], d['f']
			except:
				return print(f"Pickle: inform data (sk, f)")

			frame = cv2.imdecode(frame, cv2.IMREAD_COLOR)
			self.cam_surf = pygame.image.frombuffer(frame, (CAM_WIDTH, CAM_HEIGHT), "BGR").convert()
